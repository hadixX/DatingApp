...
export class UserService {
  ...
  getMessage(id: number, page?, itemsPerPage?,messageContainer?){
    ...
    return this.http.get<Message[]>(this.baseUrl + 'user/'+id+'/messages',{observe:'response',params}).pipe(
      map(response => {
        paginatedResult.result = response.body;
        if(response.headers.get('Pagination') != null){
          paginatedResult.pagination = JSON.parse(response.headers.get('Pagination'));
  ...
  getMessagesThread(id:number,recipiantId:number){
    return this.http.get<Message[]>(this.baseUrl+'user/'+id+'/messages/thread/'+recipiantId);
  }
